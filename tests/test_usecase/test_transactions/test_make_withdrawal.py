...
def test_call_with_default_params(make_balance):
    ...
    txn = txn_usecase.make_withdrawal(balance=balance, amount=100)

    assert txn.balance == balance
    assert txn.balance_remainder == -100
    assert txn.direction == FundsDirection.EXPENSE
...
@pytest.mark.usefixtures("with_memory_database")
def test_call_with_full_params(make_account, make_balance, make_expense_category):
    account = make_account()
    balance = make_balance(account=account)
    category = make_expense_category(account=account)
...
def test_balance_value_checking(make_account):
    ...
    with pytest.raises(check.ValidationError):
        txn_usecase.make_withdrawal(balance=account, amount=100)


@pytest.mark.usefixtures("with_memory_database")
def test_amount_value_checking(make_balance):
    balance = make_balance()

    with pytest.raises(check.ValidationError):
        txn_usecase.make_withdrawal(balance=balance, amount=0)
        txn_usecase.make_withdrawal(balance=balance, amount=-1)
        txn_usecase.make_withdrawal(balance=balance, amount=0.5)
...
def test_category_value_checking(
    ...
    category1 = make_expense_category(account=account1)
    category2 = make_expense_category(account=account2)

    with pytest.raises(check.ValidationError):
        txn_usecase.make_withdrawal(balance=balance1, amount=1, category=balance1)
        txn_usecase.make_withdrawal(
            balance=balance1,
            amount=1,
            category=make_income_category(account=account1),
        )
        txn_usecase.make_withdrawal(balance=balance1, amount=1, category=category2)
        txn_usecase.make_withdrawal(balance=balance2, amount=1, category=category1)
...
def test_comment_value_checking(make_balance):
    ...
    with pytest.raises(check.ValidationError):
        txn_usecase.make_withdrawal(balance=balance, amount=1, comment=0)
...
def test_created_at_value_checking(make_balance):
    ...
    with pytest.raises(check.ValidationError):
        txn_usecase.make_withdrawal(balance=balance, amount=1, created_at=0)
...
def test_first_txn_calculation(make_balance):
    ...
    txn = txn_usecase.make_withdrawal(balance=balance, amount=2050)
...
def test_second_txn_calculation(make_balance):
    ...
    txn0 = txn_usecase.make_withdrawal(balance=balance, amount=1050)
    assert txn0.balance_remainder == -1050
    assert txn0.amount == 1050
    assert models.Balance.get().amount == -1050

    txn1 = txn_usecase.make_withdrawal(balance=balance, amount=2025)
...
def test_sametime_created_txn_calculation(make_balance):
    ...
    now = datetime.now()
    one_minnute_ago = now - timedelta(minutes=1)

    txn0 = txn_usecase.make_withdrawal(
        balance=balance, amount=1050, created_at=one_minnute_ago
    )
    assert txn0.balance_remainder == -1050
    assert txn0.amount == 1050
    assert models.Balance.get().amount == -1050
    ...
    txn2 = txn_usecase.make_withdrawal(balance=balance, amount=500, created_at=now)
...
def test_txn_calculation_between_txns(make_balance):
    ...
    txn1 = txn_usecase.make_withdrawal(
...
@pytest.mark.usefixtures("with_memory_database")
def test_sametime_created_txn_calculation_between_txns(make_balance):
    balance = make_balance()
    ...
    txn3 = txn_usecase.make_withdrawal(balance=balance, amount=200, created_at=now)
