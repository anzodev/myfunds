...
def test_call_with_default_params(
    ...
    t0, t1, _ = execution_timing(business.make_replenishment)(
    ...
    assert txn.type_ == constants.TransactionType.REPLENISHMENT
...
def test_call_with_full_params(
    new_account, new_currency, new_balance, new_replenishment_group
):
    account = new_account()
    balance = new_balance(account, new_currency())
    txn_group = new_replenishment_group(account=account)
    ...
    assert txn.amount == 100
    assert txn.comment == "test replenishment"
...
def test_balance_param_verify(new_account):
    ...
    with pytest.raises(ValueError):
        business.make_replenishment(balance=account, amount=100)
...
def test_amount_param_verify(new_account, new_currency, new_balance):
    ...
    with pytest.raises(ValueError):
        business.make_replenishment(balance=balance, amount=0)
        business.make_replenishment(balance=balance, amount=-1)
        business.make_replenishment(balance=balance, amount=0.5)
...
def test_txn_group_param_verify(
    ...
    txn_replenishment_group1 = new_replenishment_group(account=account1)
    txn_replenishment_group2 = new_replenishment_group(account=account2)

    with pytest.raises(ValueError):
        business.make_replenishment(balance=balance1, amount=1, txn_group=balance1)
        business.make_replenishment(
            balance=balance1, amount=1, txn_group=new_withdrawal_group(account=account1)
        )
        business.make_replenishment(
            balance=balance1,
            amount=1,
            txn_group=txn_replenishment_group2,
        )
        business.make_replenishment(
            balance=balance2,
            amount=1,
            txn_group=txn_replenishment_group1,
        )
...
def test_comment_param_verify(new_account, new_currency, new_balance):
    ...
    with pytest.raises(ValueError):
        business.make_replenishment(balance=balance, amount=1, comment=0)
...
def test_created_at_param_verify(new_account, new_currency, new_balance):
    ...
    with pytest.raises(ValueError):
        business.make_replenishment(balance=balance, amount=1, created_at=0)
...
def test_first_txn_calc(new_account, new_currency, new_balance):
    ...
    txn = business.make_replenishment(balance=balance, amount=2050)
...
def test_second_txn_calc(new_account, new_currency, new_balance):
    ...
    txn0 = business.make_replenishment(balance=balance, amount=1050)
    assert txn0.balance_remainder == 1050
    assert txn0.amount == 1050
    assert models.Balance.get().amount == 1050

    txn1 = business.make_replenishment(balance=balance, amount=2025)
...
def test_sametime_txn_calc(new_account, new_currency, new_balance):
    ...
    txn0 = business.make_replenishment(
        balance=balance, amount=1050, created_at=utc_1m_ago
    )
    assert txn0.balance_remainder == 1050
    assert txn0.amount == 1050
    assert models.Balance.get().amount == 1050
    ...
    txn2 = business.make_replenishment(balance=balance, amount=500, created_at=utc_now)
...
def test_txn_calc_between_txns(new_account, new_currency, new_balance):
    ...
    txn1 = business.make_replenishment(
...
def test_sametime_txn_calc_between_txns(new_account, new_currency, new_balance):
    ...
    txn3 = business.make_replenishment(balance=balance, amount=200, created_at=utc_now)
